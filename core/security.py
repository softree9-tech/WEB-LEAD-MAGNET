...
def is_safe_url(url: str) -> bool:
    ...
    try:
        ...
        # Resolve hostname to all associated IPs (v4 and v6)
        # This provides protection against standard and dual-stack SSRF.
...
def safe_request(method, url, **kwargs):
    """
    Wrapper around requests.request that manually handles redirects
    and validates every hop against is_safe_url to prevent SSRF.
    """
    max_redirects = kwargs.pop('max_redirects', 5)
    # We must handle redirects manually
    ...
    for i in range(max_redirects + 1):
        ...
        if response.is_redirect:
            ...
            if not location or i >= max_redirects:
                # Return the redirect response if we reached max or no location
                return response
...
def validate_website(url: str) -> dict:
    ...
    try:
        ...
        # Using safe_request to prevent SSRF via redirects
