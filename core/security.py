...
def _resolve_hostname(hostname: str) -> list:
    """Resolve hostname to a list of IP addresses (IPv4 and IPv6)."""
    try:
...
def validate_website(url: str) -> dict:
    ...
    if not url:
        return {
            "valid": False,
            "error": "Invalid domain — unable to locate website.",
            "url": url,
        }
    ...
    try:
        ...
        if body_chunk:
            ...
            if len(stripped) < 200 and not re.search(
                r"<(div|section|main|article|p|h[1-6])", stripped, re.IGNORECASE
            ):
                return {
                    "valid": False,
                    "error": "Invalid domain — unable to locate website.",
        ...
        if response.status_code >= 400:
            status_text = (
                "page not found" if response.status_code == 404 else "access error"
            )
            return {
                "valid": True,
