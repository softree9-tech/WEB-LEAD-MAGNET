...
export function GeoFAQ() {
  ...
  return (
    <section id="faqs" className="relative py-28 px-6" style={{ background: 'linear-gradient(180deg, #050816, #081020)' }}>
      <div className="max-w-3xl mx-auto relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="text-center mb-16">
          <motion.p variants={fadeUp} custom={0} className="text-orange-400 text-sm font-semibold uppercase tracking-widest mb-3">Support</motion.p>
          <motion.h2 variants={fadeUp} custom={1} className="text-3xl md:text-4xl font-extrabold text-white mb-4">Frequently Asked Questions</motion.h2>
        ...
        <div className="space-y-3">
          {faqs.map((faq, i) => (
            <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i} variants={fadeUp}
              ...
              >
                <span className={`font-semibold text-sm transition-colors ${openIdx === i ? 'text-orange-400' : 'text-white group-hover:text-orange-300'}`}>
                  {faq.q}
                </span>
                <ChevronDown size={18} className={`text-text-muted transition-transform duration-300 flex-shrink-0 ml-4 ${openIdx === i ? 'rotate-180 text-orange-400' : ''}`} />
              </button>
...
export function GeoCTA({ onScrollToHero }) {
  return (
    <section className="relative py-28 px-6 overflow-hidden">
      {/* Ambient glows */}
      <div className="geo-ambient-glow w-[500px] h-[500px] bg-orange-500/8 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 absolute" />
      <div className="geo-ambient-glow w-72 h-72 bg-orange-500/5 bottom-0 left-20 absolute" />

      <div className="max-w-3xl mx-auto relative z-10 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <p className="text-orange-400 text-sm font-semibold uppercase tracking-widest mb-4">Get Started</p>
...
export function GeoFooter() {
  ...
  return (
    <footer className="border-t border-white/[0.04] py-12 px-6" style={{ background: '#040714' }}>
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6">
            {links.map(l => (
              <a key={l.href} href={l.href} onClick={(e) => { e.preventDefault(); document.querySelector(l.href)?.scrollIntoView({ behavior: 'smooth' }); }}
                className="text-text-muted text-xs hover:text-white transition-colors no-underline">
                {l.label}
              </a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3">
            {['X', 'in', 'G'].map((s) => (
              <div key={s} className="w-8 h-8 rounded-lg bg-white/[0.03] border border-white/[0.06] flex items-center justify-center text-text-muted text-xs font-medium hover:border-orange-500/20 hover:text-orange-400 transition-all cursor-pointer">
                {s}
              </div>
            ))}
          </div>
