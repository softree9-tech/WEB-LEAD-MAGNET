    ...
    <section id="geo-report" className="relative py-28 px-6">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="text-center mb-16">
          <motion.p variants={fadeUp} custom={0} className="text-orange-400 text-sm font-semibold uppercase tracking-widest mb-3">Preview</motion.p>
          <motion.h2 variants={fadeUp} custom={1} className="text-3xl md:text-4xl font-extrabold text-white mb-4">A Glimpse of Your GEO Report</motion.h2>
        ...
        <div className="geo-report-cards grid grid-cols-1 md:grid-cols-3 gap-6">
          {cards.map((card, i) => (
            <motion.div key={card.title} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
              variants={fadeUp} whileHover={{ y: -4, borderColor: 'rgba(255,107,0,0.25)' }}
              ...
              {card.scoreValue && (
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-orange-400 text-sm font-bold">{card.scoreValue}/100</span>
                </div>
              )}
              {card.items && (
...
export function GeoHowItWorks() {
  ...
  return (
    <section id="how-it-works" className="relative py-28 px-6" style={{ background: 'linear-gradient(180deg, #050816, #081020)' }}>
      <div className="max-w-7xl mx-auto relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="text-center mb-16">
          <motion.p variants={fadeUp} custom={0} className="text-orange-400 text-sm font-semibold uppercase tracking-widest mb-3">Process</motion.p>
          <motion.h2 variants={fadeUp} custom={1} className="text-3xl md:text-4xl font-extrabold text-white mb-4">How It Works</motion.h2>
          ...
          {steps.map((step, i) => (
            <motion.div key={step.num} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
              ...
              <motion.div whileHover={{ scale: 1.08 }}
                className="w-16 h-16 rounded-2xl bg-gradient-to-br from-orange-500/15 to-orange-500/5 border border-orange-500/20 flex items-center justify-center mx-auto mb-5 relative z-10 group-hover:border-orange-500/40 transition-all">
                <span className="text-orange-400 text-lg font-extrabold">{step.num}</span>
              </motion.div>
...
export function GeoFeaturesGrid() {
  ...
  return (
    <section id="features" className="relative py-28 px-6">
      <div className="geo-ambient-glow w-80 h-80 bg-orange-500/5 bottom-0 right-0 absolute" />
      <div className="max-w-7xl mx-auto relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="text-center mb-16">
          <motion.p variants={fadeUp} custom={0} className="text-orange-400 text-sm font-semibold uppercase tracking-widest mb-3">Capabilities</motion.p>
          <motion.h2 variants={fadeUp} custom={1} className="text-3xl md:text-4xl font-extrabold text-white mb-4">Comprehensive AI Visibility Analysis</motion.h2>
        ...
        <div className="geo-features-grid grid grid-cols-1 md:grid-cols-3 gap-5">
          {features.map((f, i) => (
            <motion.div key={f.title} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
              ...
              <div className="w-10 h-10 rounded-xl bg-orange-500/8 border border-orange-500/15 flex items-center justify-center mb-4 group-hover:bg-orange-500/12 transition-colors">
...
export function GeoWhyMatters() {
  ...
  return (
    <section id="why-geo" className="relative py-28 px-6" style={{ background: 'linear-gradient(180deg, #081020, #050816)' }}>
      <div className="geo-ambient-glow w-96 h-96 bg-orange-500/5 top-1/2 left-0 -translate-y-1/2 absolute" />
      <div className="max-w-7xl mx-auto relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="text-center mb-16">
          <motion.p variants={fadeUp} custom={0} className="text-orange-400 text-sm font-semibold uppercase tracking-widest mb-3">The Shift</motion.p>
          <motion.h2 variants={fadeUp} custom={1} className="text-3xl md:text-4xl font-extrabold text-white mb-4">Why GEO Matters</motion.h2>
