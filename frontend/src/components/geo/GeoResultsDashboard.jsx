...
function computeGeoScores(data) {
  ...
  if (data.services_detected) {
    (Array.isArray(data.services_detected) ? data.services_detected : [data.services_detected]).forEach(s => entities.push({ type: 'Service', value: s }));
  }
  if (data.industry) entities.push({ type: 'Industry', value: data.industry });
  if (data.company_type) entities.push({ type: 'Company Type', value: data.company_type });
  if (data.target_audience) entities.push({ type: 'Target Audience', value: data.target_audience });
  if (data.schema_types?.length) {
    data.schema_types.forEach(s => entities.push({ type: 'Schema', value: s }));
  }

  // Recommendations
  const recs = [];
  if (!data.seo_og) recs.push({ title: 'Add Open Graph markup', impact: 'High', difficulty: 'Easy', desc: 'Enable rich previews when AI engines share your content.' });
  if (!data.seo_canonical) recs.push({ title: 'Set canonical URL', impact: 'Medium', difficulty: 'Easy', desc: 'Prevent duplicate content confusion for AI crawlers.' });
  if (schemaScore < 60) recs.push({ title: 'Add Organization schema', impact: 'High', difficulty: 'Medium', desc: 'Help AI engines understand your brand entity structure.' });
  if (!data.has_newsletter) recs.push({ title: 'Add FAQ structured data', impact: 'High', difficulty: 'Medium', desc: 'Significantly improves AI citation probability by ~40%.' });
  if (data.cta !== 'Strong') recs.push({ title: 'Strengthen semantic relevance', impact: 'High', difficulty: 'Medium', desc: 'Improve content clarity for AI content extraction.' });
  recs.push({ title: 'Improve service entity structure', impact: 'Medium', difficulty: 'Medium', desc: 'Map your services to recognized entity categories.' });
  if (aeo < 50) recs.push({ title: 'Improve AI citation signals', impact: 'High', difficulty: 'Hard', desc: 'Add authority markers that AI engines use for content ranking.' });
  if (recs.length < 4) recs.push({ title: 'Add Product/Service schema', impact: 'Medium', difficulty: 'Easy', desc: 'Enable AI platforms to surface your offerings accurately.' });

  return {
    aiVisibility, seo, aeo, ux, trust,
    chatgpt, gemini, perplexity, claude,
    schemaScore, citationScore,
    entities: entities.slice(0, 12),
    recommendations: recs.slice(0, 6),
    domain: data.website?.replace(/^https?:\/\/(www\.)?/, '').replace(/\/$/, '') || '',
  };
}
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        >{score}</motion.span>
        <span className="text-text-muted text-[10px] font-semibold uppercase tracking-wider">{label || grade}</span>
      </div>
      {/* Pulse ring */}
      <motion.div
    ...
    >
      ...
      <div className="w-full h-2 rounded-full bg-[var(--color-ring-track,rgba(10,10,26,0.18))] overflow-hidden mb-3">
        <motion.div
          ...
          initial={{ width: 0 }}
          animate={{ width: `${score}%` }}
          transition={{ duration: 1.5, ease: 'easeOut', delay: 0.3 + delay * 0.1 }}
        />
      </div>
      <p className="text-text-muted text-xs">
        {score >= 70 ? 'Strong visibility' : score >= 45 ? 'Moderate visibility' : 'Low visibility'} — {score >= 70 ? 'well-positioned for citations' : 'optimization recommended'}
      </p>
    </motion.div>
  );
}
    ...
    <div className="max-w-7xl mx-auto px-6 pb-24 pt-8 space-y-16">
        ...
        <div className="geo-glass p-10 md:p-14 relative overflow-hidden" style={{ borderColor: 'rgba(255,88,18,0.12)' }}>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-10 relative z-10">
            ...
            <motion.div variants={fadeUp} custom={0} className="flex-shrink-0">
              <ScoreRing score={s.aiVisibility} size={160} strokeWidth={10} label="AI Score" />
            </motion.div>
            ...
            <div className="flex-1 text-center lg:text-left">
              ...
              {/* Mini score bars */}
              <motion.div variants={fadeUp} custom={4} className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <PlatformCard name="Gemini" score={s.gemini} color="#3B82F6" delay={1} />
          <PlatformCard name="Perplexity" score={s.perplexity} color="#A855F7" delay={2} />
          <PlatformCard name="Claude" score={s.claude} color="#F59E0B" delay={3} />
        </div>
      </section>
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider"
                    style={{ background: `${impactColors[rec.impact]}15`, color: impactColors[rec.impact], border: `1px solid ${impactColors[rec.impact]}30` }}>
                    {rec.impact} Impact
                  </span>
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider"
                    style={{ background: `${difficultyColors[rec.difficulty]}15`, color: difficultyColors[rec.difficulty], border: `1px solid ${difficultyColors[rec.difficulty]}30` }}>
                    {rec.difficulty}
                  </span>
                </div>
              </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            ...
            { label: 'Semantic Clarity', score: Math.round((s.aeo + s.ux) / 2), icon: Brain },
            { label: 'AI Indexing', score: Math.round((s.seo + s.schemaScore) / 2), icon: Search },
          ].map((item, i) => (
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
        ...
        <p className="text-text-muted text-xs">
          Report generated {new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} • {s.domain} • Confidential
        </p>
      </motion.div>
    </div>
  );
