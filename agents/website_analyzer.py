...
def website_analyzer_agent(state: AgentState) -> AgentState:
    ...
    if url:
        ...
        try:
            ...
            # --- POST-BROWSER ANALYSIS (CPU/IO Bound) ---
            ...
            # Use pre-parsed soup for broken link check
            link_data = count_broken_links(html, url, soup=soup)
            broken_links = link_data["broken_list"]
            total_links = link_data["total"]

            # Perform all other BeautifulSoup analysis using the same soup instance
            ...
            # Clean for LLM (Modifies soup in-place)
