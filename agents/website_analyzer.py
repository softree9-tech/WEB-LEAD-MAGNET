...
def check_single_link(link: str, session: requests.Session = None) -> str:
    ...
    try:
        ...
        res = _req.head(link, timeout=10, allow_redirects=True, headers=headers, verify=False)
        
        # If server blocks HEAD requests, fallback to a lightweight streamed GET
        if res.status_code in [403, 405, 401, 301, 302, 999]:
            res = _req.get(link, timeout=10, allow_redirects=True, headers=headers, stream=True, verify=False)
            res.raw.close()
...
def count_broken_links(html: Union[str, BeautifulSoup], base_url: str) -> Dict:
    ...
    if links_to_test:
        with requests.Session() as session:
            # Configure pooling to match thread count
            adapter = HTTPAdapter(pool_connections=15, pool_maxsize=15)
            session.mount('http://', adapter)
            session.mount('https://', adapter)

            with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
                # Use partial or lambda to pass the session to check_single_link
    ...
    if url:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Optimization: Extract from conversion_elements instead of re-searching DOM
