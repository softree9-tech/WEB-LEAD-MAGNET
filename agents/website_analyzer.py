...
def count_broken_links(soup: BeautifulSoup, base_url: str) -> list:
    ...
    broken_list = []
    if links_to_test:
        # Use a session with connection pooling to speed up multiple requests
        with requests.Session() as session:
            adapter = requests.adapters.HTTPAdapter(pool_connections=10, pool_maxsize=15)
            session.mount("http://", adapter)
            session.mount("https://", adapter)

            with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
                # Pass the session to each worker to enable connection reuse
...
def check_lead_capture(soup: BeautifulSoup, html_lower: str) -> bool:
    ...
    # Check for popup/modal form triggers and chat widgets
    popup_signals = [
...
def check_cta_presence(soup: BeautifulSoup) -> bool:
...
def check_conversion_elements(soup: BeautifulSoup, html_lower: str) -> dict:
    ...
    # 7. CTA Presence
    has_cta = check_cta_presence(soup)
    ...
    if url:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Single BeautifulSoup instance for all downstream checks
