...
def check_single_link(link: str, session=None) -> str:
    ...
    try:
        ...
        # Use provided session for connection pooling
    ...
    if links_to_test:
        ...
        with requests.Session() as session:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
                # Pass session to check_single_link
                future_to_url = {executor.submit(check_single_link, url, session): url for url in links_to_test}
...
def extract_tech_stack(html: str, headers: dict, html_lower: str = "") -> str:
    if not html_lower:
...
def check_conversion_elements(soup: BeautifulSoup, html: str = "", html_lower: str = "") -> dict:
    ...
    # 7. CTA Presence
    has_cta = check_cta_presence(soup, html, html_lower)
    ...
    if url:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Close browser immediately after capturing visual data to save RAM
                browser.close()

                # Optimize analysis: single BS4 instance and pre-computed lowercase HTML
                html_lower = html.lower()
                soup = BeautifulSoup(html, "html.parser")

                tech_stack = extract_tech_stack(html, headers, html_lower)
                last_modified = extract_last_modified(headers, html)
                link_data = count_broken_links(html, url, soup)
                broken_links = link_data["broken_list"]
                total_links = link_data["total"]

                analytics_data = check_analytics(html, html_lower)

                has_lead_capture = check_lead_capture(soup, html, html_lower)
                conversion_elements = check_conversion_elements(soup, html, html_lower)
                # Consolidate: use results from conversion_elements to avoid redundant calls
