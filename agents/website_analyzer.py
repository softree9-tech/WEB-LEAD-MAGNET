...
def check_single_link(session: requests.Session, link: str) -> str:
...
def count_broken_links(soup: BeautifulSoup, base_url: str) -> dict:
    ...
    if links_to_test:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
            future_to_url = {executor.submit(check_single_link, session, url): url for url in links_to_test}
...
def check_conversion_elements(soup: BeautifulSoup, html_lower: str = "") -> dict:
    ...
    mailtos = soup.find_all("a", href=lambda href: href and href.startswith("mailto:"))
    tels = soup.find_all("a", href=lambda href: href and ("tel:" in href))
    has_direct_contact = bool(mailtos or tels)
    ...
    if url:
        ...
        try:
            with sync_playwright() as p:
                ...
                has_lead_capture = conversion_elements["contact_form"] or conversion_elements["chat_whatsapp"] or conversion_elements["popup_lead_capture"] or conversion_elements["direct_contact"]
