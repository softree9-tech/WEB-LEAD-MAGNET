...
def check_single_link(link: str, session: requests.Session = None) -> str:
    ...
    try:
        ...
        if res.status_code in [403, 405, 401, 301, 302, 999]:
            getter_full = session.get if session else requests.get
            res = getter_full(link, timeout=10, allow_redirects=True, headers=headers, stream=True, verify=False)
            res.raw.close()
    ...
    if links_to_test:
        ...
        with requests.Session() as session:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
                # Pass the session to check_single_link for connection pooling
