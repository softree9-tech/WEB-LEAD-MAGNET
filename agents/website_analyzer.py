...
def check_single_link(link: str, session: requests.Session = None) -> str:
    ...
    try:
        ...
        request_func = session.head if session else requests.head
        res = request_func(link, timeout=10, allow_redirects=True, headers=headers, verify=False)
...
def count_broken_links(html: str, base_url: str, soup: BeautifulSoup = None) -> list:
    ...
    if links_to_test:
        ...
        # Enable connection pooling for the 15 threads
        adapter = HTTPAdapter(pool_connections=10, pool_maxsize=15)
        session.mount("http://", adapter)
        session.mount("https://", adapter)

        with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
            # Pass session to check_single_link for connection reuse
    ...
    if url:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Optimize: pre-calculate html_lower and pre-parse BeautifulSoup for reuse
                html_lower = html.lower()
                soup = BeautifulSoup(html, "html.parser")
            ...
            try:
                ...
                # Optimize fallback path as well
                html_lower = html.lower()
                soup = BeautifulSoup(html, "html.parser")
