...
def check_single_link(link: str, session: requests.Session = None) -> str:
    ...
    try:
        ...
        if res.status_code in [403, 405, 401, 301, 302, 999]:
            req_func_get = session.get if session else requests.get
            res = req_func_get(link, timeout=10, allow_redirects=True, headers=headers, stream=True, verify=False)
            res.raw.close()
...
def count_broken_links(html: Union[str, BeautifulSoup], base_url: str, session: requests.Session = None) -> dict:
    ...
    broken_list = []
    if links_to_test:
        # Define a wrapper to pass the session to check_single_link
        def _check(link):
            return check_single_link(link, session=session)

        with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
            results = executor.map(_check, links_to_test)
    ...
    if url:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Use a small pool to match our thread count
                adapter = requests.adapters.HTTPAdapter(pool_connections=15, pool_maxsize=15)
                link_session.mount('http://', adapter)
                link_session.mount('https://', adapter)

                link_data = count_broken_links(soup, url, session=link_session)
