    ...
    if url:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Parallelize I/O bound link checking while we continue with screenshots
                broken_links_future = executor.submit(count_broken_links, soup, url)
                ...
                # Performance: Reuse existing conversion results to avoid redundant DOM traversal
            ...
            try:
                ...
                # Parallelize link checking in fallback mode too
                broken_links_future = executor.submit(count_broken_links, soup, url)
        ...
        # Wait for broken links results
        broken_links = []
        total_links = 0
        if broken_links_future:
            try:
                link_data = broken_links_future.result(timeout=15)
                broken_links = link_data.get("broken_list", [])
                total_links = link_data.get("total", 0)
            except Exception as e:
                print(f"Broken Links Future Error: {e}")
