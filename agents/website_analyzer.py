...
def check_single_link(link: str, session: requests.Session = None) -> str:
    ...
    try:
        ...
        if res.status_code in [403, 405, 401, 301, 302, 999]:
            ...
            res.close() # Ensure response is closed when using stream=True
...
def count_broken_links(html: str, base_url: str) -> list:
    ...
    if links_to_test:
        ...
        # Setting pool_maxsize to match ThreadPoolExecutor max_workers
        with requests.Session() as session:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
                # Use partial to pass the session to each worker
                check_link_with_session = functools.partial(check_single_link, session=session)
                results = executor.map(check_link_with_session, links_to_test)
