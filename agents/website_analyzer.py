...
def count_broken_links(html: str, base_url: str, soup: BeautifulSoup = None) -> dict:
    ...
    if links_to_test:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
            # Wrap check_single_link to pass the session
            def check_with_session(url):
                return check_single_link(url, session=session)

            results = executor.map(check_with_session, links_to_test)
    ...
    if url:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Optimized: count_broken_links now reuses the soup object
                ...
                # Optimized: Get conversion elements once and reuse results
